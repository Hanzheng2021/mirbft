...
func (pi *pbftInstance) lead() {
	...
	//if config.Config.CrashTiming == "Straggler" {
		config.Config.BatchTimeoutMs = int(0.01666*float64(config.Config.ViewChangeTimeoutMs))
...
func (pi *pbftInstance) sendCommit(batch *pbftBatch) {
	...
	if commit.Tn > pi.localhtn {
		pi.localhtn = commit.Tn
	}
...
func (pi *pbftInstance) handleHtnmsg(htnmsg *pb.HtnMessage, msg *pb.ProtocolMessage) error {
	//logger.Debug().Int32("prelocalhtn", pi.localhtn).
	//	Msg("previous localhtn.")
	prelocalhtn := pi.localhtn
	htn := htnmsg.Htn
	if htn >= pi.localhtn {
		pi.localhtn = htn;	
	} 
	logger.Debug().Int32("htn", htn).Int32("localhtn", pi.localhtn).Int32("prelocalhtn", prelocalhtn).
		Msg("func handleHtnmsg htn.")
	return nil
}
