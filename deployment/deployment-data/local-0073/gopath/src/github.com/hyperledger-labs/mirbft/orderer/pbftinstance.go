...
func (pi *pbftInstance) handleHtnmsg(htnmsg *pb.HtnMessage, msg *pb.ProtocolMessage) error {
	...
	prelocalhtn := htnlog[pi.segment.SegID()]
	htn := htnmsg.Htn
	if htn >= prelocalhtn {
		htnlog[pi.segment.SegID()] = htn;	
	} 
	logger.Debug().Int32("prelocalhtn", prelocalhtn).Int32("htn", htn).Int32("localhtn", htnlog[pi.segment.SegID()]).
